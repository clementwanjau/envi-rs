...
use crate::constants::{CC_REG, CC_STACK, Endianess};
use crate::error::Error::{Generic, UnknownCallingConvention};
use crate::memory::{MemoryDef, MemoryObject};
use crate::operands::OpCode;
use crate::registers::RegisterContext;
use crate::Result;
use crate::utils::{parse_bits, parse_bytes};
...
impl<'a> dyn Emulator + 'a {
    ...
    fn get_meta(&self, name: &str, default: Option<i32>) -> i32 {
        let data = self.get_emulator_data();
        match data.metadata.get(name) {
            Some(val) => *val,
            None => {
                default.unwrap_or_else(|| 0)
            }
        }
    }
...
pub trait CallingConvention {
    ...
    fn set_precall_args(&self, emulator: &mut dyn Emulator, args: Vec<i32>) {
        let data = self.get_data();
    ...
    fn exec_call_return(&self, emulator: &dyn Emulator, value: i32, argc: Option<i32>) -> i32 {
        ...
        //emulator.write_memory_format(sp, format!("<P"), &[value]);
        ret_addr
    }
}
