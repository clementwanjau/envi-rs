...
use crate::constants::RMETA_NMASK;
...
pub trait RegisterContext{
    fn get_register_context_data(&self) -> &mut RegisterContextData;
    ...
    fn set_register_snap(&self, regs: Vec<i32>) {
        self.get_register_context_data()._rctx_vals = regs;
    }
    ...
    fn set_is_dirty(&mut self, dirty: bool) {
        self.get_register_context_data()._rctx_dirty = dirty;
    }
    ...
    fn load_reg_metas(&self, reg_metas: Vec<MetaRegister>, stat_metas: Option<Vec<StatusMetaRegister>>) {
        let data = self.get_register_context_data();
        data._rctx_reg_metas = reg_metas.clone();
        for meta in reg_metas {
            self.add_meta_register(meta);
        }
    ...
    fn add_meta_register(&self, meta_register: MetaRegister) {
    ...
    fn set_register(&mut self, index: i32, mut value: i32) {
        ...
        if rindx != index {
            value = self._xlate_to_native_reg(index, value);
        }
